...
class Agent():
    ...
    def choose_action(self, state):
        ...
        return np.argmax(self.qtable[state]) # return the index of the highest Q-value
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        current_value = self.qtable[state, action]  # current Q-value for the state/action couple
        next_max = np.max(self.qtable[next_state])  # next best Q-value
        
        # Compute the new Q-value with the Bellman equation
        self.qtable[state, action] = current_value + self.learning_rate*(reward + self.gamma*next_max - current_value)
