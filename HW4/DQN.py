...
class Agent():
    ...
    def learn(self):
        ...
        batch_state, batch_action, batch_reward, batch_next_state, done = self.buffer.sample(self.batch_size)

        # 3. Forward the data to the evaluate net and the target net.
        actions = torch.tensor(np.array(batch_action).reshape(len(batch_action), 1), dtype=torch.long)
        rewards = torch.tensor(np.array(batch_reward).reshape(len(batch_reward), 1), dtype=torch.float)

        # Compute Q-values for current states and selected actions
        current_q_value = self.evaluate_net(torch.tensor(np.array(batch_state), dtype=torch.float)).gather(1, actions)

        # Compute Q-values for next states using the target net
        next_q_values = self.target_net(torch.tensor(np.array(batch_next_state), dtype=torch.float)).detach()
        target_q_values = rewards + self.gamma * next_q_values.max(1).values.unsqueeze(-1) # target Q value
        for i in range(len(done)):
            if done[i]:
                target_q_values[i][0] = 0

        # 4. Compute the loss with MSE.
        loss = F.mse_loss(current_q_value, target_q_values)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            # Begin your code
            state_torch = torch.tensor(state, dtype=torch.float32)  # Convert state to a tensor
            q_values = self.evaluate_net(state_torch)  # Get Q-values for the state from the evaluate network
            if np.random.uniform(0, 1) < 1-self.epsilon:  # Choose a random action with epsilon probability
    ...
    def check_max_Q(self):
        ...
        # Get Q-values for the initial state from the target network
        q_values = self.target_net(initial_state_tensor).detach()
        # Calculate the max Q value
        max_q = q_values.max(0).values.unsqueeze(-1)
        max_q = float(max_q[0])
        return max_q
        # End your code
