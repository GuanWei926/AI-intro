...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        indices = (
            SelectPercentile(
                f_classif, percentile=10
            )  # Select the top 10% ranked features
        ...
        weights = np.zeros(len(dataset))  # create list filled with zero
        for i in range(len(dataset)):
            if labels[i] == 1:
        ...
        for t in range(self.T):
            ...
            # update weights
            accuracy = []
            for x, y in zip(iis, labels): # Link iis and lables together using the zip function.
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        for j in range(len(features)):
            ...
            for threshold in range(int(mid-50), int(mid+51), 2):
                for polarity in [-1, 1]:
                    #polarity = 1
                    clf = WeakClassifier(features[j],threshold, polarity)
                    predictions = []
                    for i in range(len(iis)):
                        # Calculate error for the current classifier
                        res = 1 if polarity * featureVals[j, i] < polarity * threshold else 0
                        predictions.append(res)
                        #predictions.append(clf.classify(img))
                    error = np.sum(weights * (np.array(predictions) != np.array(labels)))

                    # Update best classifier if the current error is lower
                    if error < bestError:
                        bestError = error
                        bestClf = clf
