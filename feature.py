...
class HaarFeature:
    ...
    def computeFeature(self, ii):
        '''
        print(ii.shape)
        for pos in self.posRegions:
            print(pos)
            print(pos.computeFeature(ii))
        print(sum([pos.computeFeature(ii) for pos in self.posRegions]))
        '''
        return sum([pos.computeFeature(ii) for pos in self.posRegions]) - sum([neg.computeFeature(ii) for neg in self.negRegions]) #1D array
